<template>
  ...
  <div class="container">
    ...
    <div class="unsuported" v-if="curStep === -1">
      <h2>Seu navegador não suporta a captura de fotos</h2>
      <p>
        Por favor, utilize um navegador mais atualizado ou um dispositivo diferente para enviar sua
        redação.
      </p>
    </div>
    <div class="tirar-foto" v-if="curStep === 0">
      <video autoplay></video>
      <div class="final">
        <button class="btn-padrao" @click="virarCamera" v-if="canFlipCamera">Virar câmera</button>
        <button class="btn-padrao" @click="tirarFoto">Tirar foto</button>
      </div>
...
const startCamera = () => {
  // se o navegador não suportar getUserMedia, exibe um alerta
  if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
    curStep.value = -1
  }
  const video = document.querySelector('video')

  // Call getUserMedia directly from navigator.mediaDevices
  ...
  navigator.mediaDevices.enumerateDevices().then((devices) => {
    ...
    canFlipCamera.value = videoDevices.length > 1
  })
}
...
const enviarFoto = () => {
  ...
  fetch(import.meta.env.VITE_APP_API_URL + '/correnem-llm-ms/llm/extrair-texto', {
    ...
    .then((response) => {
      ...
      localStorage.setItem('redacao', response.extractedText)
      // redirect to /nova-redacao/texto
